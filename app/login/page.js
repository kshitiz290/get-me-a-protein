...
const Login = () => {
  ...
  useEffect(() => {
    document.title = "Login - Get Me a Protein"
    if (session) {
  ...
  let handlesubmit = async(e) =>{
    e.preventDefault();
    if(!form.username){
      seterror({...error,username:"Username is required",password:password?"":"Password is required"});
      return
    }
    if(!form.password){
      seterror({...error,password:"Password is required",username:username?"":"Username is required"});
      return;
    }
    const res = await logged_in(form);
    if(res.error){
  ...
  return (
    <>
      <div className='text-white py-6 container mx-auto'>
        <h1 className='md:text-3xl text-xl text-center font-bold'>Login to get your fans support you!</h1>
      </div>

      <div className='login-content login-content flex flex-col md:flex-row rounded-lg shadow w-3/4 mx-auto justify-center'>
          <div className="login-form  w-full md:w-1/3 p-4 space-y-2">
            <h1 className="md:text-xl text-lg font-bold leading-tight tracking-tight text-gray-900  dark:text-white">
              Login in with your Credentials 
            </h1>
            <form className="space-y-4 " onSubmit={handlesubmit}>
              <div>
                <label htmlFor="username" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your Username</label>
                <input type="username" name="username" id="username" className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value={form.username?form.username:""} onChange={handlechange}  placeholder="name@company.com" required="" />
                {error.username && <p className='text-sm text-red-500 mt-1'>{error.username}</p>}
              </div>
              <div>
                <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                <input type="password" name="password" id="password" placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value={form.password?form.password:""} onChange={handlechange}  required="" />
              ...
              <div className="flex items-center justify-between">
                ...
                <Link href="#" className="text-sm font-medium text-purple-600 hover:underline dark:text-purple-500">Forgot password?</Link>
              </div>
              ...
              <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                Don’t have an account yet? <Link href="/signup" className="font-medium text-purple-600 hover:underline dark:text-purple-500">Sign up</Link>
              </p>
            </form>
            <ToastContainer/>
          </div>
          <div className='text-white text-xl text-center md:py-32 font-bold md:m-4'>or</div>
        <div className="social-login-buttons ">
          <div className="flex w-1/2 flex-col gap-2 md:min-h-screen p-6 py-6 md:py-20">


            <button  onClick={()=>{signIn("google")}}
              className="flex items-center w-64 bg-white border border-gray-300 rounded-lg shadow-md max-w-xs px-6 py-2 text-sm font-medium  hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
            ...
            {/* <button
              className="flex items-center  w-64 border bg-white border-gray-300 rounded-lg shadow-md max-w-xs px-6 py-2 text-sm font-medium  hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
              <svg className="h-6 w-6 mr-2" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
